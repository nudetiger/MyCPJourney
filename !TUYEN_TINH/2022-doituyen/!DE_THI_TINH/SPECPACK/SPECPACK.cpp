...
int lis(int n, vector<pii>& v, vector<int>& index) {

    vector<int> seq;
    int i = 0;
    for(i = 0; i < n; i ++) {
        if(index[i] == 0 || v[i].first == 0 || v[i].first == 1) continue;
...
void solve() {
    ...
    for(int i = 2; i * i <= m; i += 2) {
        ...
        for(int j = ind; j < n; j++) {
            if(index[v[j].second] == 0) continue;
            if(v[j].first % i == 0) {
                index[v[j].second] = 0;
            } 
        }
